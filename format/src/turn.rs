...
impl Turn {
    ...
    pub fn memory_id(&self) -> Result<MemoryId> {
        if self.turn_id.memory_layer() != MemoryLayer::Turn {
    ...
    pub fn with_row_id(mut self, row_id: u64) -> Self {
        self.turn_id = MemoryId::new(MemoryLayer::Turn, row_id);
        self
    }
...
impl TurnTable {
    ...
    pub async fn insert(&self, turns: &mut [Turn]) -> Result<()> {
        if turns.is_empty() {
            return Ok(());
        }
        if turns
            ...
            .any(|turn| turn.turn_id.memory_point() != u64::MAX)
        {
            return Err(Error::invalid_input(
                "turn insert requires pending turn ids",
            ));
        }
    ...
    #[allow(dead_code)]
    pub async fn get_turn(&self, turn_id: u64) -> Result<Option<Turn>> {
        let Some(dataset) = self.access.try_open().await? else {
            return Ok(None);
        };
        let batch = dataset
    ...
    #[allow(dead_code)]
    pub(crate) async fn get_turns(&self, turn_ids: &[u64]) -> Result<Vec<Turn>> {
    ...
    #[cfg(test)]
    async fn load_latest_turn(&self, query: &TurnQuery) -> Result<Option<Turn>> {
    ...
    async fn load_session_turns(&self, query: &TurnQuery) -> Result<Vec<Turn>> {
        let Some(dataset) = self.access.try_open().await? else {
            return Ok(Vec::new());
        };
        let batch = dataset
    ...
    async fn load_all_turns(&self) -> Result<Vec<Turn>> {
        let Some(dataset) = self.access.try_open().await? else {
            return Ok(Vec::new());
        };
        let batch = dataset
    ...
    pub async fn timeline_turns(
        ...
        if memory_id.memory_layer() != MemoryLayer::Turn {
            return Err(Error::invalid_input(format!(
