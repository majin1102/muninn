...
use super::codec::{record_batch_to_sessions, sessions_to_reader};
use crate::maintenance::{
    SESSION_TEXT_COLUMN, cleanup_dataset, compact_dataset, ensure_semantic_vector_index,
    ensure_session_fts_index, ensure_session_identity_index, optimize_session,
};
...
impl SessionTable {
    ...
    pub async fn ensure_dataset(&self) -> Result<LanceDataset> {
        ...
        self.access.write(sessions_to_reader(Vec::new())?).await
    }
    ...
    pub async fn validate_dimensions(&self, expected_dimensions: usize) -> Result<()> {
        ...
        let actual_dimensions = session_vector_dimensions(&dataset)?;
        if actual_dimensions != expected_dimensions {
            return Err(Error::invalid_input(format!(
                "session dimension mismatch: muninn.json expects {expected_dimensions}, but the existing session table stores {actual_dimensions}; update providers.embedding.<name>.dimensions or rebuild the session table"
            )));
        }
        Ok(())
    }
    ...
    pub async fn ensure_vector_index(&self, target_partition_size: usize) -> Result<bool> {
        ...
        let fts_created = ensure_session_fts_index(&mut dataset).await?;
        let identity_created = ensure_session_identity_index(&mut dataset).await?;
    ...
    pub async fn optimize(&self, merge_count: usize) -> Result<bool> {
        ...
        optimize_session(&mut dataset, merge_count).await
    }

    pub async fn list(&self, limit: Option<usize>) -> Result<Vec<Session>> {
        let Some(dataset) = self.access.try_open().await? else {
            return Ok(Vec::new());
        };
        ...
        record_batch_to_sessions(&batch)
    }

    pub async fn get(&self, identities: &[SessionIdentity]) -> Result<Vec<Session>> {
        ...
        let rows = record_batch_to_sessions(&batch)?;
    ...
    pub async fn upsert(&self, rows: Vec<Session>) -> Result<()> {
        if rows.is_empty() {
            return Ok(());
        }
        ...
        if let Some(dataset) = self.access.try_open().await? {
            ...
            job.execute_reader(sessions_to_reader(rows)?).await?;
        } else {
            self.access.write(sessions_to_reader(rows)?).await?;
        }
        Ok(())
    }

    pub async fn replace_all(&self, rows: Vec<Session>) -> Result<()> {
        validate_unique_identities(&rows)?;
        let reader = sessions_to_reader(rows)?;
    ...
    async fn validate_query_vector(&self, query_vector: &[f32]) -> Result<()> {
        ...
        if query_vector.len() != actual_dimensions {
            return Err(Error::invalid_input(format!(
                "session query vector dimension mismatch: query vector has {}, but the session table stores {actual_dimensions}",
                query_vector.len(),
            )));
        }
        Ok(())
    ...
    async fn nearest(&self, query_vector: &[f32], limit: usize) -> Result<Vec<Session>> {
        ...
        if let Ok(scanner) = dataset.scan().nearest("vector", &query_vector, limit) {
            if let Ok(batch) = scanner.try_into_batch().await {
                ...
                return record_batch_to_sessions(&batch);
            }
        }
        ...
        let mut rows = record_batch_to_sessions(&batch)?;
    ...
    async fn full_text(&self, query: &str, limit: usize) -> Result<Vec<Session>> {
        ...
        let query = FullTextSearchQuery::new(normalized_query.to_string())
            .with_column(SESSION_TEXT_COLUMN.to_string())?
...
fn identity_key(row: &Session) -> IdentityKey {
    (
        row.project.clone(),
        row.agent.clone(),
...
fn validate_unique_identities(rows: &[Session]) -> Result<()> {
    let mut seen = HashSet::new();
    for row in rows {
        let key = identity_key(row);
        if !seen.insert(key) {
            return Err(Error::invalid_input(format!(
                "duplicate session identity: project='{}', agent='{}', session_id='{}'",
...
fn merge_ranked(vector_rows: Vec<Session>, fts_rows: Vec<Session>, limit: usize) -> Vec<Session> {
    let mut scores: HashMap<IdentityKey, f32> = HashMap::new();
    let mut rows: HashMap<IdentityKey, Session> = HashMap::new();
...
fn add_rank(
    ...
    rows: &mut HashMap<IdentityKey, Session>,
    rank: usize,
    row: Session,
) {
    let key = identity_key(&row);
...
async fn fallback_full_text(
    ...
    let mut scored = record_batch_to_sessions(&batch)?
        .into_iter()
        .filter_map(|row| {
...
fn lexical_score(row: &Session, tokens: &[String]) -> usize {
...
fn session_vector_dimensions(dataset: &LanceDataset) -> Result<usize> {
    let vector = dataset.schema().field("vector").ok_or_else(|| {
        Error::invalid_input(
            "session table schema is invalid: missing vector column; rebuild the session table",
        )
    })?;
        ...
        DataType::FixedSizeList(item, dimensions) if item.data_type() == &DataType::Float32 => {
            if dimensions <= 0 {
                return Err(Error::invalid_input(
                    "session table schema is invalid: vector dimension must be positive; rebuild the session table",
                ));
            }
            Ok(dimensions as usize)
        }
        actual => Err(Error::invalid_input(format!(
            "session table schema is incompatible: expected vector column type FixedSizeList<Float32, N>, found {actual:?}; rebuild the session table"
        ))),
    }
}
...
mod tests {
    ...
    use super::{Session, SessionIdentity, SessionTable};
    ...
    fn table() -> SessionTable {
        SessionTable::new(TableOptions::local(data_root().unwrap()).unwrap())
    }
    ...
    async fn upsert_rejects_duplicate_session_identity_before_writing() {
        ...
        assert!(
            empty_table_error
                .to_string()
                .contains("duplicate session identity"),
            "{empty_table_error}"
        );
    ...
    async fn replace_all_rejects_bad_vector_without_deleting_existing_rows() {
        ...
        assert!(
            error.to_string().contains("invalid session vector"),
            "{error}"
        );
        let rows = table.list(None).await.unwrap();
        assert_eq!(rows.len(), 1);
    ...
    async fn search_rejects_mismatched_query_vector_dimensions() {
        ...
        assert!(
            error
                .to_string()
                .contains("session query vector dimension mismatch"),
            "{error}"
        );
    }
    ...
    async fn replace_all_rejects_duplicate_session_identity() {
        ...
        assert!(
            error.to_string().contains("duplicate session identity"),
            "{error}"
        );
        let rows = table.list(None).await.unwrap();
        assert_eq!(rows.len(), 1);
    ...
    async fn validate_dimensions_rejects_wrong_embedding_config() {
        ...
        assert!(
            error.to_string().contains("session dimension mismatch"),
            "{error}"
        );
    }
}
