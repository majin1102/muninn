...
pub fn current_observer_config() -> Result<Option<ObserverConfig>> {
    Ok(load_munnai_config()?.and_then(|config| {
...
fn resolve_task_llm_config(config: &MunnaiConfig, task: LlmTask) -> Option<LlmFileConfig> {
...
fn load_munnai_config() -> Result<Option<MunnaiConfig>> {
    ...
    let parsed = serde_json::from_str::<MunnaiConfig>(&raw).map_err(|error| {
...
pub(crate) fn config_path() -> Option<PathBuf> {
    Some(munnai_home().join("settings.json"))
}

pub(crate) fn munnai_home() -> PathBuf {
    if let Ok(path) = std::env::var("MUNNAI_HOME") {
        return PathBuf::from(path);
    }

    if let Some(home) = std::env::var_os("HOME") {
        return PathBuf::from(home).join(".munnai");
    }
...
pub(crate) fn llm_test_env_guard() -> std::sync::MutexGuard<'static, ()> {
    ...
    let isolated_home = std::env::temp_dir().join("munnai-test-home");
    fs::create_dir_all(&isolated_home).expect("create isolated munnai home");
  ...
  "observer": {
    ...
    unsafe {
        std::env::set_var("MUNNAI_HOME", isolated_home.as_os_str());
    }
...
#[cfg(test)]
pub(crate) fn write_test_munnai_config(
    path: &std::path::Path,
    ...
    fs::write(
        path,
        serde_json::to_string_pretty(&Value::Object(root)).expect("serialize test munnai config"),
    )
    .expect("write test munnai config");
}
...
mod tests {
    ...
    fn reads_turn_and_observer_config_from_turn_and_observer_sections() {
        ...
        unsafe {
            std::env::set_var("MUNNAI_HOME", home.as_os_str());
        }
    ...
    #[test]
    fn uses_munnai_home_when_set() {
        let _guard = llm_test_env_guard();
        let dir = tempfile::tempdir().unwrap();
        let munnai_home = dir.path().join("custom-home");
        fs::create_dir_all(&munnai_home).unwrap();
        fs::write(
            munnai_home.join("settings.json"),
            r#"{
        ...
        .unwrap();
        unsafe {
            std::env::set_var("MUNNAI_HOME", &munnai_home);
        }
    ...
    #[test]
    fn falls_back_to_default_home_settings_when_munnai_home_is_missing() {
        let _guard = llm_test_env_guard();
        let dir = tempfile::tempdir().unwrap();
        let fake_home = dir.path().join("home");
        let user_config = fake_home.join(".munnai").join("settings.json");
    ...
    fn reads_storage_config_from_settings() {
        ...
        assert_eq!(storage.uri, "s3://example-bucket/munnai");
        assert_eq!(
            storage
                .storage_options
                .as_ref()
