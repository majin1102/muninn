...
use crate::llm::config::{EmbeddingConfig, MunnaiConfig, SemanticIndexFileConfig};
...
pub fn semantic_index_config() -> Result<EmbeddingConfig> {
    let file_config = load_munnai_config()?.and_then(|config| config.semantic_index);
...
pub fn semantic_index_config_from_raw(raw: &str) -> Result<EmbeddingConfig> {
    let parsed = parse_munnai_config(raw, "provided settings config")?;
...
pub fn watchdog_config() -> Result<WatchdogConfig> {
    let file_config = load_munnai_config()?.and_then(|config| config.watchdog);
...
pub fn data_root() -> Result<PathBuf> {
    Ok(crate::llm::config::munnai_home())
}
...
fn load_munnai_config() -> Result<Option<MunnaiConfig>> {
    ...
    let parsed = parse_munnai_config(&raw, &path.display().to_string())?;
    Ok(Some(parsed))
}
...
fn parse_munnai_config(raw: &str, source: &str) -> Result<MunnaiConfig> {
    serde_json::from_str::<MunnaiConfig>(raw)
