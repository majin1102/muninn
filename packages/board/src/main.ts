...
function resolveApiBase(): string {
  ...
  if (fromQuery) {
    localStorage.setItem('munnai.board.apiBase', fromQuery);
    return trimTrailingSlash(fromQuery);
  }

  const fromStorage = localStorage.getItem('munnai.board.apiBase');
  if (fromStorage) {
...
function resolveDataMode(): DataMode {
  ...
  if (fromQuery === '1') {
    localStorage.setItem('munnai.board.dataMode', 'tree');
    return 'tree';
  }

  if (fromQuery === '0') {
    localStorage.setItem('munnai.board.dataMode', 'live');
    return 'live';
  }

  const stored = localStorage.getItem('munnai.board.dataMode');
  if (stored === 'card') {
    return 'card';
  }
...
function restoreLeftWidth(): number {
  const stored = Number(localStorage.getItem('munnai.board.leftWidth'));
  ...
  return `
    <header class="topbar">
      <div class="brand-block">
        ...
        <h1 class="topbar-title">Munnai Board</h1>
      </div>
      <div class="topbar-actions">
        ...
        <a class="topbar-version topbar-action-link" href="https://github.com/majin1102/munnai/releases" target="_blank" rel="noreferrer">Version: ${escapeHtml(state.backendVersion ?? DEFAULT_BACKEND_VERSION)}</a>
        <a class="github-link" href="https://github.com/majin1102/munnai" target="_blank" rel="noreferrer" aria-label="Open GitHub repository">
...
function bindViewEvents() {
  ...
  document.querySelectorAll<HTMLElement>('[data-action="set-data-mode"]').forEach((element) => {
    element.addEventListener('click', async () => {
      ...
      state.dataMode = mode;
      localStorage.setItem('munnai.board.dataMode', mode);
      state.agents = [];
      state.sessionGroups = {};
      state.sessionTurns = {};
      state.observings = [];
...
function bindResizer() {
  ...
  resizer.onmousedown = (event: MouseEvent) => {
    ...
    const onMove = (moveEvent: MouseEvent) => {
      ...
      localStorage.setItem('munnai.board.leftWidth', String(state.leftWidth));
      render();
    };
