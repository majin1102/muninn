...
import { parseObserverSubtree } from '../observer/markdown.js';
...
export async function observeCwdScope(input: ObserveCwdScopeInput): Promise<ParsedObserverDocument> {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    const raw = await generateObserverText({
      system: template.system,
...
async function generateObserverText(params: {
  system: string;
  prompt: string;
  signal?: AbortSignal;
  cwdScope: string;
...
async function writeObserverTrace(event: {
  database?: string;
  input: unknown;
  prompt: {
    system: string;
  ...
  document?: ParsedObserverDocument;
  toolResults?: unknown[];
}): Promise<void> {
