...
test('thread observing prompt organizes entity extractions by questions', () => {
  ...
  assert.match(prompt.system, /observer that rewrites part of a cross-session observation document/i);
  assert.match(prompt.system, /root anchor is fixed/);
  assert.match(prompt.system, /observation outline is a navigation map, not evidence/);
  assert.match(prompt.system, /Rewrite content is the selected prior observation snapshot linked to the current extractions/);
  assert.match(prompt.system, /new` extraction units/);
  assert.match(prompt.system, /changed` extraction units/);
  assert.match(prompt.system, /get_observation\(id\)/);
  assert.match(prompt.system, /root-to-node path and the complete subtree under that node/);
  assert.match(prompt.system, /never returns sibling branches outside that path\/subtree/);
  assert.match(prompt.system, /up to 5 semantically plausible bottom-level candidate headings/);
  assert.match(prompt.system, /outline path, not only local heading text/);
  assert.match(prompt.system, /get_observation guidance/);
  assert.match(prompt.system, /Prefer leaf candidates first/);
  assert.match(prompt.system, /full heading path/);
  assert.match(prompt.system, /Inspect multiple leaf candidates separately/);
  assert.match(prompt.system, /parent headings as scope signals/);
  assert.match(prompt.system, /non-leaf candidate only when the rewrite likely needs that whole section/);
  assert.match(prompt.system, /Do not inspect a parent merely because several candidate leaves share it/);
  assert.match(prompt.system, /Refs guidance/);
  assert.match(prompt.system, /full set of extraction ids/);
  assert.match(prompt.system, /not just the current task/);
  assert.match(prompt.system, /rewriting, merging, or moving leaf content/);
  assert.match(prompt.system, /keep or move the supporting refs with that content/);
  ...
  assert.doesNotMatch(prompt.system, /Keep observations concise/);
  assert.doesNotMatch(prompt.system, /without duplicating leaf-level evidence/);
  assert.doesNotMatch(prompt.system, /Do not compress an extraction by weakening/);
  assert.doesNotMatch(prompt.system, /Do not expand the whole root/);
  assert.match(prompt.system, /smallest subtree/);
  assert.doesNotMatch(prompt.system, /Do not rewrite unrelated branches/);
  assert.doesNotMatch(prompt.system, /Sections outside the returned subtree remain unchanged/);
  assert.match(prompt.system, /same observed scope/);
  assert.match(prompt.system, /3-8 words/);
  assert.match(prompt.system, /50-300 characters/);
  assert.match(prompt.system, /4-10 words/);
  assert.match(prompt.system, /100-500 characters/);
  assert.match(prompt.system, /leaf heading has no children and must declare refs/);
  assert.match(prompt.system, /Do not invent observation ids/);
  ...
  assert.match(prompt.userTemplate, /Observation outline:/);
  assert.match(prompt.userTemplate, /Rewrite content:/);
...
test('thread observing prompt uses generic recall-ready memory guidance', () => {
  ...
  assert.match(system, /\[Extraction\]` captures what should be remembered/);
  assert.match(system, /source object, subject, wording, or relation/);
  assert.match(system, /specific and answerable/);
  assert.match(system, /\[Context\]` captures the local source context needed to understand or locate the extraction/);
  ...
  assert.match(system, /the relation between details/);
...
test('observing prompt preserves the current extraction schema', () => {
  ...
  assert.match(system, /responses, judgments, reactions, or feedback/);
