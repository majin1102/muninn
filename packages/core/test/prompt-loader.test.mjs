...
test('thread observing prompt uses generic recall-ready memory guidance', () => {
  ...
  assert.match(system, /Observation changes: the only way to add, update, merge, or delete observations/);
...
test('observing prompt preserves the current observation schema', () => {
  ...
  assert.match(system, /read-only existing observations/i);
  assert.match(system, /Do not return final observations/i);
  ...
  assert.match(system, /add: create a new observation from fragments/);
  assert.match(system, /update: rewrite one existing observation when new evidence clarifies, corrects, time-normalizes, or extends it/);
  assert.match(system, /merge: combine existing observations when the result is one clearer observation/);
  assert.match(system, /delete: remove an existing observation only when it is low-value, duplicate, unsupported, contradicted, or replaced/);
  assert.match(system, /evidence refines one observation and introduces another independent observation/);
  assert.match(system, /Do not change existing observations just because current fragments do not mention them/);
  assert.match(system, /merge/);
  assert.match(system, /update/);
  assert.match(system, /delete/);
  assert.match(system, /Id usage/);
  assert.match(system, /Use `observingContent\.observations\[\]\.id` for `update\.observationId`, `merge\.observationIds`, and `delete\.observationId`/);
  assert.match(system, /Use `fragments\[\]\.turns\[\]\.turnId` for `add\.references` and `update\.references`/);
  assert.match(system, /Do not merge conflicting observations; keep unresolved conflicts in `openQuestions`/);
  assert.doesNotMatch(system, /"observations": \[/);
