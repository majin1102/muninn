...
- 写入路径已经收敛到“提交完整 turn”
- 当前正式写入口是 `POST /api/v1/turn/capture`
- `sessionId` 只是逻辑归属参考，不是严格生命周期边界
- 当前 OpenClaw plugin 只使用：
  - `after_tool_call`
  - `agent_end`
- `after_tool_call` 负责聚合 `toolCalls` / `artifacts`
- `agent_end` 负责从最终 `messages` 中提取 `prompt` / `response`，并一次提交完整 turn
