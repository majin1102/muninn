...
- [ ] `conv-30 Q3`: observer / recall 需要更好合并同一决策链里的 trigger、decision、motivation。现象是 `lost job -> start business` 和 `dance studio motivation` 都被召回，但最终答案只覆盖 passion/motivation，漏掉 job loss trigger。
