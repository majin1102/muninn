...
Muninn MVP 1 要验证的核心产品价值只有一件事：

- agent 在真实工作过程中产生的上下文，能否被 Muninn 自动沉淀为后续可复用的记忆

这个阶段不是为了验证 Muninn 的完整长期架构，而是为了验证最小产品闭环是否成立。

## 产品定义

Muninn 是 agent 的记忆层。

在 MVP 1 中，Muninn 应该做到：

- 从 OpenClaw 的真实执行过程中接收增量上下文
- 将这些上下文整理为 session 记忆
- 从 session 中进一步蒸馏出 observation 记忆
- 将整理和精炼后的记忆产物沉淀为可被 OpenClaw 直接召回的记忆索引
- 提供一个只读的界面，让人可以查看 Muninn 生成出来的记忆
...
如果只有原始记录，没有 observation，那么这还不算完整验证了 Muninn 的价值。
