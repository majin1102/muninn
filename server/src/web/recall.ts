...
export type RecallStreamParams = {
  query: string;
  provider: string;
...
export type AgentRecallParams = RecallStreamParams;
...
export async function* recallStream(params: RecallStreamParams): AsyncIterable<string> {
  ...
  yield* generateTextStreamWithConfig(config, {
    ...
    prompt: recallPrompt(query, params.results),
...
export async function* recallEvents(params: RecallStreamParams): AsyncIterable<AgentRecallStreamEvent> {
  try {
    for await (const text of recallStream(params)) {
      if (text) {
...
function recallPrompt(query: string, results: SearchSessionResult[]): string {
  return [
    'User question:',
    query,
    '',
...
export const agentRecall = recallStream;
export const agentRecallEvents = recallEvents;
