...
test('capture policy is stored in capture.json and defaults disabled', async (t) => {
  ...
  const stored = JSON.parse(await readFile(path.join(home, 'capture.json'), 'utf8'));
  assert.equal(stored.capture.agents.codex, true);
  assert.deepEqual(stored.capture.projects.codex, {
    'github.com/legacy/repo': true,
    'github.com/lance-format/lance': true,
  });

  assert.equal(await isCaptureEnabled('codex', 'github.com/legacy/repo'), true);
