...
test('session tree uses project agent session identity instead of cwd for row keys', async () => {
  ...
  const stateSource = await readFile(new URL('../../web/src/lib/session-content-state.ts', import.meta.url), 'utf8');
  const apiSource = await readFile(new URL('../../web/src/lib/api.ts', import.meta.url), 'utf8');
  const serverSource = await readFile(new URL('../src/web/sessions.ts', import.meta.url), 'utf8');
