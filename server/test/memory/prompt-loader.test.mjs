...
test('thread session memory prompt uses generic recall-ready memory guidance', () => {
  ...
  assert.match(system, /Record only durable memory that future agents would benefit from without re-reading the transcript/);
  assert.match(system, /Skip routine progress, chronological narration, repeated status updates/);
...
test('session memory prompt preserves the current extraction schema', () => {
  ...
  assert.match(system, /`## Signals` is a rolling prioritized set of high-value guidance for future sessions/);
  assert.match(system, /Do not append turn-by-turn/);
  assert.match(system, /explicit guidance about future behavior/);
  assert.match(system, /Keep Signals that would change future agent behavior even if summarized elsewhere/);
  assert.match(system, /inside it use `### Guidance`, `### Skills`, and `### Open Questions` subsections/);
  assert.match(system, /Each subsection has an independent maximum of 5 top-level bullets/);
  assert.match(system, /Sort each subsection by weight, highest first/);
  assert.match(system, /Skills may use nested bullets to describe workflow triggers, steps, checks, and expected output/);
  assert.match(system, /Signal types/);
  assert.match(system, /Signal weights/);
  assert.match(system, /Use `\[1\]` to `\[5\]`; higher numbers sort first/);
  assert.match(system, /For each batch, set new signal weight to `\[1\]` if supported once, or `\[2\]` if supported multiple times; increase existing signal weight by the same amount/);
  assert.match(system, /Cap weight at `\[5\]`/);
  ...
  assert.match(system, /Extraction summaries are compact recall capsules: 1-3 sentences, maximum 100 tokens/);
  assert.match(system, /Content is optional detail\/evidence space, not a transcript; maximum 800 tokens/);
  assert.match(system, /Source evidence and citations/);
  ...
  assert.match(system, /Open Questions: unresolved decisions, blockers, or user confirmations that affect future work; remove once resolved/);
...
test('session memory prompt uses raw turns and returns snapshot content document', () => {
  ...
  assert.match(system, /## Signals\n\n### Guidance\n\n- \[2\] The user repeatedly emphasized the report export filename rule/);
  assert.match(system, /### Skills\n\n- \[1\] Report export failure triage:/);
  assert.match(system, /Trigger: export job failure\./);
  assert.match(system, /Steps: check job logs first, then object storage permissions\./);
  assert.match(system, /Recording: keep the final root cause and fix, not every inspection step\./);
  assert.match(system, /### Open Questions\n\n- \[1\] The report export audit field name is still unresolved/);
