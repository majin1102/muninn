...
test('thread session memory prompt uses generic recall-ready memory guidance', () => {
  ...
  assert.doesNotMatch(system, /Use the dominant language of current-batch user instructions for natural-language signals and Skill Details/);
  ...
  assert.match(system, /whole-session metadata/);
  assert.match(system, /Summary should orient future recall/);
  assert.match(system, /what this session is about, important decisions\/changes, current state, and open questions or next checks if any/);
  assert.doesNotMatch(system, /not extraction metadata/);
  assert.doesNotMatch(system, /latest topic\/fix\/extraction/);
  assert.match(system, /short broad\/stable whole-session label/);
  assert.match(system, /overall session scope/);
  assert.match(system, /avoid UUIDs, paths, or `Session <id>` defaults/);
  assert.match(system, /optional broad\/stable whole-session update/);
  assert.doesNotMatch(system, /if any extraction exists/);
  assert.match(system, /Session title: usually 4-8 tokens; session summary: max 256 tokens/);
  ...
  assert.match(system, /More text is not better; keep content dense and retrieval-oriented/);
