...
test('hitCandidates respects project and session scope from enriched recall metadata', async () => {
  ...
  const candidates = __testing.hitCandidates([
    recallHit({ contextId: 'ext:1', sessionId: 'search-a', project: 'muninn', agent: 'codex' }),
    recallHit({ contextId: 'ext:2', sessionId: 'search-b', project: 'muninn', agent: 'codex' }),
    recallHit({ contextId: 'ext:3', sessionId: 'search-a', project: 'lance', agent: 'codex' }),
  ], {
    projectKeys: ['muninn'],
...
test('hitCandidates keeps same raw session ids separate across projects and agents', async () => {
  ...
  const grouped = __testing.groupCandidates(__testing.hitCandidates([
    recallHit({ contextId: 'ext:1', sessionId: 'same-session', project: 'muninn', agent: 'codex' }),
    recallHit({ contextId: 'ext:2', sessionId: 'same-session', project: 'lance', agent: 'codex' }),
    recallHit({ contextId: 'ext:3', sessionId: 'same-session', project: 'muninn', agent: 'claude' }),
...
test('hitCandidates ignores non-extraction hits for app recall', async () => {
  const { __testing } = await loadSearchServer();
  const candidates = __testing.hitCandidates([
    recallHit({ contextId: 'session:1' }),
    recallHit({ contextId: 'ext:1' }),
  ], {});

  assert.equal(candidates.length, 1);
  assert.equal(candidates[0].contextId, 'ext:1');
});
...
function recallHit(overrides) {
  return {
    contextId: overrides.contextId ?? 'ext:1',
