...
export function entriesFromFallback(
  ...
  if (entries.length === 0) {
    const split = splitPreviewFallback(turn.preview);
    if (split) {
      entries.push({
        type: 'message',
        message: {
...
function splitPreviewFallback(preview: string | undefined): { prompt: string; response: string } | null {
  if (!preview) {
    return null;
  }
  const parts = preview
  ...
  return {
    prompt: stripPreviewLabel(parts[0]!, 'Prompt'),
    response: stripPreviewLabel(parts.slice(1).join('\n\n'), 'Response'),
  };
}

function stripPreviewLabel(value: string, label: 'Prompt' | 'Response'): string {
