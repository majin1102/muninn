...
test('right chat view opens around the selected turn and expands by direction', async () => {
  ...
  const turns = Array.from({ length: 121 }, (_, index) => ({
    contextId: `turn:${index + 1}`,
  }));

  const defaultWindow = chatTurnWindow(turns, null);
  assert.equal(DEFAULT_CHAT_INITIAL_TURN_COUNT, 16);
  assert.deepEqual(defaultWindow.turns.map((turn) => turn.contextId), turns.slice(0, 16).map((turn) => turn.contextId));
  ...
  assert.deepEqual(expandedDefaultWindow.turns.map((turn) => turn.contextId), turns.slice(0, 32).map((turn) => turn.contextId));
  ...
  assert.deepEqual(initial.turns.map((turn) => turn.contextId), turns.slice(40, 81).map((turn) => turn.contextId));
  ...
  assert.deepEqual(expandedBefore.turns.map((turn) => turn.contextId), turns.slice(24, 81).map((turn) => turn.contextId));
  ...
  assert.deepEqual(expandedAfter.turns.map((turn) => turn.contextId), turns.slice(40, 97).map((turn) => turn.contextId));
